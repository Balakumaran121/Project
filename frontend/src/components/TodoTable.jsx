            ...
            finalList?.length ? finalList?.map((todo) => (
              <tr className=' ' key={todo?._id}>
                ...
                <td className='px-4 py-2 whitespace-nowrap text-white space-x-4'>  
                  <button onClick={()=>handleSelectTodo(todo,"status")} className={`px-2 flex items-center py-[6px] ${todo?.status === "1" ? "bg-green-500 hover:bg-green-600" : "bg-red-500 hover:bg-red-600"} rounded-md text-white text-md font-semibold overflow-hidden cursor-pointer`}> 
                    In-Complete
                    <span className=' flex px-1.5 justify-center items-center rounded-[6px] text-[20px]'>{todo?.status === "0" ? (<span > &#x2715;</span> ):(<span className='text-white'>&#10003;</span> )} </span>
                  </button>
                </td>
                <td className='px-4 py-2 whitespace-nowrap text-white space-x-4'>
                 ...
                 {
                  todo?.status === "1" && (<button onClick={()=>handleSelectTodo(todo._id,"delete")} className='bg-red-500 hover:bg-red-600 text-white font-semibold cursor-pointer rounded-md px-6 py-[9px]'>Delete</button> )
                 }
