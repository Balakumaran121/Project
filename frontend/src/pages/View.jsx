...
const View = () => {
  ...
  return (
    <div className='h-screen bg-zinc-950 text-white'>
        ...
        <h1 className='font-semibold pl-70 pt-20 flex items-center gap-2 cursor-pointer '> <span className='hover:bg-cyan-500 hover:text-white rounded-full' onClick={handleBack}><ChevronLeft /></span> View Task</h1>
       <div className='flex w-full'>
        <div  className='w-4/12'>

       <img src="" alt="" className='w-96 ml-60 mt-10 h-96 rounded-4xl  bg-cyan-500   '/>
        </div>
       ...
       <p className=' mt-2 text-lg font-medium'>{data?.data?.priority||[]}</p>
        ...
        <Trash2 className='hover:bg-cyan-500 rounded-full p-2' size={70}/>
        </div>
       </div>
    </div>
  )
